...
void signal_paused_propertieschanged(void)
{
        ...
        g_variant_builder_add(builder, "{sv}", "paused",
                              g_variant_new_boolean(status.pause_level != 0));
        g_variant_builder_add(builder, "{sv}", "pauseLevel",
                              g_variant_new_uint32(status.pause_level));

        g_dbus_connection_emit_signal(
            dbus_conn, NULL, FDN_PATH, PROPERTIES_IFAC, "PropertiesChanged",
            g_variant_new("(sa{sv}as)", DUNST_IFAC, builder, invalidated_builder),
            NULL);
                              ...
                              gpointer user_data)
{
        ...
        else
        {
                ...
                {
                        LOG_W(
                            "DBUS_SESSION_BUS_ADDRESS is blank. Is the dbus session configured "
                            "correctly?");
                }
...
void signal_history_removed(guint id)
{
        ...
        g_dbus_connection_emit_signal(dbus_conn, NULL, FDN_PATH, DUNST_IFAC,
                                      "NotificationHistoryRemoved", body, &err);
...
void signal_history_cleared(guint n)
{
        ...
        g_dbus_connection_emit_signal(dbus_conn, NULL, FDN_PATH, DUNST_IFAC,
                                      "NotificationHistoryCleared", body, &err);
