...
int string_parse_color(const char* s, struct color* ret)
{
        ...
        {
                LOG_W(
                    "A color string should start with '#' and contain at least 3 hex "
                    "characters");
                return false;
        }
...
int get_setting_id(const char* key, const char* section)
{
        ...
        if (error_code == -2)
        {
                LOG_W("Setting %s is in the wrong section (%s, should be %s)", key, section,
...
/**
 * @warning We do minimal checks in this function so the values should be
 * sanitized somewhere else.
 */
int string_parse_length(void* ret_in, const char* s)
{
        ...
        if (!s_stripped)
        {
                ...
                {
                        LOG_W(
                            "Specify either a single value or two comma-separated values "
                            "between parentheses");
                        return false;
                }
        ...
        if (len != 2)
        {
                g_strfreev(s_arr);
                LOG_W(
                    "Specify either a single value or two comma-separated values between "
                    "parentheses");
                return false;
        }
        ...
        switch (setting.type)
        {
                ...
                case TYPE_PATH:;
                        ...
                        *(char**)target = string_to_path(g_strdup(value));
                        ...
                        if (!g_shell_parse_argv(*(char**)target, NULL,
                                                (char***)setting.parser_data, &error))
                        {
                                LOG_W(
                ...
                case TYPE_LENGTH:
                        ...
                        {
                                LOG_M(
                                    "Using legacy offset syntax NxN, you should switch to the new "
                                    "syntax (N, N)");
                                return true;
                        }
                        ...
                        {
                                LOG_M(
                                    "Setting 'height' has changed behaviour after dunst 1.12.0, see "
                                    "https://dunst-project.org/release/#v1.12.0.");
                                LOG_M(
                                    "Legacy height support may be dropped in the future. If you want "
                                    "to hide this message transition to");
                                LOG_M(
                                    "'height = (0, X)' for dynamic height (old behaviour equivalent) "
                                    "or to 'height = (X, X)' for a fixed height.");
                ...
                default:
                        LOG_W("Setting type of '%s' is not known (type %i)", setting.name,
                              setting.type);
                        return false;
        }
}
        ...
        for (int i = 0; i < ini->section_count; i++)
        {
                ...
                for (int j = 0; j < curr_section.entry_count; j++)
                {
                        ...
                        if (setting_id < 0)
                        {
                                if (setting_id == -1)
                                {
                                        LOG_W("Setting %s in section %s doesn't exist", curr_entry.key,
                                              curr_section.name);
                                }
                                continue;
                        }
                        ...
                        if (is_special_section(curr_section.name))
                        {
                                if (is_rule)
                                {
                                        ...
                                        {
                                                LOG_D("Adding rule '%s = %s' to special section %s", curr_entry.key,
                                                      curr_entry.value, curr_section.name);
                                                set_rule(curr_setting, curr_entry.value, curr_section.name);
                                        }
                                        else
                                        {
                                                LOG_W(
                                                    "Cannot use filtering rules in special section. Ignoring %s "
                                                    "in section %s.",
                                                    curr_entry.key, curr_section.name);
                                        }
                                }
                                else
                                {
                        ...
                        else
                        {
                                ...
                                LOG_D("Adding rule '%s = %s' to section %s", curr_entry.key,
                                      curr_entry.value, curr_section.name);
                                set_rule(curr_setting, curr_entry.value, curr_section.name);
                        }
                }
...
char* cmdline_get_string_offset(const char* key, const char* def, int start,
                                int* found)
{
...
char* cmdline_get_string(const char* key, const char* def,
...
char* cmdline_get_path(const char* key, const char* def,
                       const char* description)
{
        cmdline_usage_append(key, "string", description);
...
char** cmdline_get_list(const char* key, const char* def,
                        const char* description)
{
...
double cmdline_get_double(const char* key, double def,
                          const char* description)
{
...
void cmdline_usage_append(const char* key, const char* type,
                          const char* description)
{
        char* key_type;
        if (STR_FULL(type))
...
const char* cmdline_create_usage(void) { return usage_str; }
