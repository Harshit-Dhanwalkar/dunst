...
char* color_to_string(struct color c, char buf[10])
{
        ...
        g_snprintf(buf, 10, "#%02x%02x%02x%02x", (int)(c.r * 255), (int)(c.g * 255),
...
void gradient_pattern(struct gradient* grad)
{
        ...
        {
                grad->pattern =
                    cairo_pattern_create_rgba(grad->colors[0].r, grad->colors[0].g,
                                              grad->colors[0].b, grad->colors[0].a);
        }
        else
        {
                ...
                for (size_t i = 0; i < grad->length; i++)
                {
                        ...
                        cairo_pattern_add_color_stop_rgba(grad->pattern, offset,
                                                          grad->colors[i].r, grad->colors[i].g,
                                                          grad->colors[i].b, grad->colors[i].a);
                }
        }
}
...
char* gradient_to_string(const struct gradient* grad)
{
        ...
        {
                j += g_snprintf(
                    buf + j, max - j, "#%02x%02x%02x%02x", (int)(grad->colors[i].r * 255),
                    (int)(grad->colors[i].g * 255), (int)(grad->colors[i].b * 255),
...
{
        bool horizontal_icon = (n->icon && (n->icon_position == ICON_LEFT ||
                                            n->icon_position == ICON_RIGHT));
                         ...
                         double scale)
{
        ...
        int progress_bar_height =
            have_progress_bar(cl) ? settings.progress_bar_height + settings.padding
                                  : 0;
        int max_text_height =
            MAX(0, settings.height.max - progress_bar_height - 2 * settings.padding);
        layout_setup_pango(cl->l, text_width, max_text_height, cl->n->word_wrap,
                           cl->n->ellipsize, cl->n->alignment);
}
                                                       ...
                                                       struct notification* n)
{
        ...
        /* markup */
        GError* err = NULL;
        pango_parse_markup(n->text_to_render, -1, 0, &(cl->attr), &(cl->text), NULL,
                           &err);
 ...
 */
static inline void draw_rect(cairo_t* c, double x, double y, double width,
                             double height, double scale)
{
        cairo_rectangle(c, round(x * scale), round(y * scale), round(width * scale),
                        round(height * scale));
}
                       ...
                       enum corner_pos corners)
{
        ...
        // cairo_stroke and not cairo_fill. Since the only case where we have partial
        // angles is the progress bar and there we use fill, maybe this can be removed
        // completely?
        ...
        // The trickiest cases to handle are when the width is less than corner_radius
        // and corner_radius * 2, because we have to split the angle for the arc in
        // the rounded corner
        ...
        if (!(skip & C_BOT_LEFT))
        {
                ...
                {
                        cairo_arc(c, x + corner_radius, y + height - corner_radius, corner_radius,
                                  bot_left_angle1, bot_left_angle2);
                }
                else
                {
        ...
        if (!(skip & C_TOP_LEFT))
        {
                ...
                {
                        cairo_arc(c, x + corner_radius, y + corner_radius, corner_radius,
                                  top_left_angle1, top_left_angle2);
                }
                else
                {
                        cairo_line_to(c, x, y);
        ...
        if (!(skip & C_TOP_RIGHT))
        {
                ...
                {
                        cairo_arc(c, x + top_x_off, y + corner_radius + top_y_off, corner_radius,
                                  top_right_angle1, top_right_angle2);
                }
                else
                {
        ...
        if (!(skip & C_BOT_RIGHT))
        {
                ...
                {
                        cairo_arc(c, x + bot_x_off, y + height - corner_radius + bot_y_off,
                                  corner_radius, bot_right_angle1, bot_right_angle2);
                }
                else
                {
...
render_background(cairo_surface_t* srf, struct colored_layout* cl,
                  struct colored_layout* cl_next, int y, int width, int height,
                  int corner_radius, enum corner_pos corners, int* ret_width,
                  double scale)
{
        int x = 0;
        ...
        cairo_set_source_rgba(c, COLOR(cl, frame.r), COLOR(cl, frame.g),
                              COLOR(cl, frame.b), COLOR(cl, frame.a));
        cairo_fill(c);

        draw_rounded_rect(c, x, y, width, height, radius_int, scale, corners);
        cairo_set_source_rgba(c, COLOR(cl, bg.r), COLOR(cl, bg.g), COLOR(cl, bg.b),
                              COLOR(cl, bg.a));
        cairo_fill(c);
            ...
            (corners & (C_BOT | _C_LAST)) == 0)
        {
                ...
                cairo_set_source_rgba(c, sep_color.r, sep_color.g, sep_color.b,
                                      sep_color.a);

                draw_rect(c, settings.frame_width, y + height, width,
                          settings.separator_height, scale);
        ...
        return cairo_surface_create_for_rectangle(
            srf, round(x * scale), round(y * scale), round(width * scale),
            round(height * scale));
}
                           ...
                           int height, double scale)
{
        ...
        if (cl->icon && cl->n->icon_position != ICON_OFF)
        {
                int image_width = get_icon_width(cl->icon, scale),
                    ...
                    image_x = width - settings.h_padding - image_width, image_y = text_y,
                ...
                cairo_set_source_surface(c, cl->icon, round(image_x * scale),
                                         round(image_y * scale));
                draw_rounded_rect(c, image_x, image_y, image_width, image_height,
                                  settings.icon_corner_radius, scale,
        ...
        if (!cl->n->hide_text)
        {
                ...
                cairo_set_source_rgba(c, COLOR(cl, fg.r), COLOR(cl, fg.g), COLOR(cl, fg.b),
                                      COLOR(cl, fg.a));
        ...
        if (have_progress_bar(cl))
        {
                 ...
                 * scaling Note: for now the bar background is drawn a little bit smaller
                 * than the fill, however this solution is not particularly solid (basically
                 * subracting a pixel or two)
                 */
                ...
                draw_rounded_rect(c, frame_x + half_frame_width + 1, frame_y,
                                  progress_width - frame_width - 2, progress_height,
                                  settings.progress_bar_corner_radius, scale,
                                  settings.progress_bar_corners);
                cairo_stroke(c);
        }
                                       ...
                                       enum corner_pos corners)
{
        ...
        cairo_surface_t* content =
            render_background(srf, cl, cl_next, dim.y, dim.w, bg_height,
                              dim.corner_radius, corners, &bg_width, scale);
                     ...
                     int* ret_x, int* ret_y)
{
        ...
        switch (settings.origin)
        {
                ...
                case ORIGIN_BOTTOM_RIGHT:
                        *ret_x =
                            scr->x + (scr->w - width) - round(settings.offset.x * draw_get_scale());
                        break;
