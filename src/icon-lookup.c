...
int load_icon_theme_from_dir(const char* icon_dir, const char* subdir_theme)
{
        ...
        icon_themes[index].dirs =
            g_malloc0_n(icon_themes[index].dirs_count, sizeof(struct icon_theme_dir));
...
void get_theme_path(void)
{
        ...
        add_paths_from_env(theme_path, "XDG_DATA_DIRS", "icons",
                ...
                if (match_size)
                {
                        ...
                        for (const char** suf = suffixes; *suf; suf++)
                        {
                                ...
                                char* icon = g_build_filename(theme->location, theme->subdir_theme,
                                                              dir.name, name_with_extension, NULL);
...
char* find_icon_in_theme_with_inherit(const char* name, int theme_index,
                                      int size)
{
        ...
        for (int i = 0; i < icon_themes[theme_index].inherits_count; i++)
        {
                ...
                icon = find_icon_in_theme(name, icon_themes[theme_index].inherits_index[i],
                                          size);
                if (icon)
                        return icon;
        }
...
char* find_icon_path(const char* name, int size)
{
        ...
        {
                char* icon =
                    find_icon_in_theme_with_inherit(name, default_themes_index[i], size);
                if (icon)
                        return icon;
        }
        return NULL;
