...
static void pixbuf_data_to_cairo_data(const unsigned char* pixels_p,
                                      ...
                                      size_t rowstride_p, size_t rowstride_c,
...
cairo_surface_t* gdk_pixbuf_to_cairo_surface(GdkPixbuf* pixbuf)
{
        ...
        pixbuf_data_to_cairo_data(
            ...
            cairo_format_stride_for_width(fmt, width), gdk_pixbuf_get_width(pixbuf),
...
static char* get_id_from_data(const uint8_t* data_pb, size_t width,
                              size_t height, size_t pixelstride,
                              size_t rowstride)
{
        ...
        {
                memcpy(data_chk + (i * rowstride_short), data_pb + (i * rowstride),
                       rowstride_short);
        }

        char* id =
            g_compute_checksum_for_data(G_CHECKSUM_MD5, data_chk, data_chk_len);
        g_free(data_chk);

        return id;
}

GdkPixbuf* get_pixbuf_from_file(const char* filename, char** id, int min_size,
                                int max_size, double scale)
{
        GError* error = NULL;
        gint w, h;
        ...
        pixbuf = gdk_pixbuf_new_from_file_at_scale(filename, round(w * scale),
                                                   round(h * scale), TRUE, &error);
                             ...
                             int min_size, int max_size)
{
        ...
        g_variant_get(data, "(iiibii@ay)", &width, &height, &rowstride, &has_alpha,
                      &bits_per_sample, &n_channels, &data_variant);
        ...
        pixbuf = gdk_pixbuf_new_from_data(data_pb, GDK_COLORSPACE_RGB, has_alpha,
                                          bits_per_sample, width, height, rowstride,
                                          icon_destroy, data_pb);
        if (!pixbuf)
        {
