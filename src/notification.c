  ...
  for (int i = 0; i < n->script_count; i++) {
    ...
    if (pid1) {
      ...
      if (pid2) {
        ...
        execlp(script, script, appname, summary, body, icon, urgency,
               (char *)NULL);
...
bool notification_is_duplicate(const struct notification *a,
  ...
  return STR_EQ(a->appname, b->appname) && STR_EQ(a->summary, b->summary) &&
         STR_EQ(a->body, b->body) && a->urgency == b->urgency &&
         (a->icon_position == ICON_OFF || b->icon_position == ICON_OFF ||
          (a->icon_id && b->icon_id
               ? STR_EQ(a->icon_id, b->icon_id)
               : (a->iconname && b->iconname ? STR_EQ(a->iconname, b->iconname)
                                             : 1)));
}
...
static void notification_private_free(NotificationPrivate *p) { g_free(p); }
...
void notification_icon_replace_path(struct notification *n,
  ...
  if (n->icon_path) {
    GdkPixbuf *pixbuf =
        get_pixbuf_from_file(n->icon_path, &n->icon_id, n->min_icon_size,
                             n->max_icon_size, draw_get_scale());
    if (pixbuf) {
...
void notification_init(struct notification *n) {
  ...
  if (!COLOR_VALID(n->colors.progress_bar))
    n->colors.progress_bar = defcolors.progress_bar;
  ...
  if (g_str_has_prefix(n->summary, "DUNST_COMMAND_")) {
    const char *msg =
        "DUNST_COMMAND_* has been removed, please switch to dunstctl. See #830 "
        "for more details. https://github.com/dunst-project/dunst/pull/830";
    LOG_W("%s", msg);
...
static void notification_format_message(struct notification *n) {
  ...
  for (char *substr = strchr(n->msg, '%'); substr && *substr;
    ...
    switch (substr[1]) {
    case 'a':
      notification_replace_single_field(&n->msg, &substr, n->appname,
                                        MARKUP_NO);
      break;
    case 's':
      notification_replace_single_field(&n->msg, &substr, n->summary,
                                        MARKUP_NO);
      break;
    case 'b':
      notification_replace_single_field(&n->msg, &substr, n->body, n->markup);
      break;
    case 'c':
      notification_replace_single_field(&n->msg, &substr, n->category,
                                        MARKUP_NO);
      break;
    case 'S':
      notification_replace_single_field(
          &n->msg, &substr, n->stack_tag ? n->stack_tag : "", MARKUP_NO);
      break;
    case 'I':
      icon_tmp = g_strdup(n->iconname);
      notification_replace_single_field(
          &n->msg, &substr, icon_tmp ? basename(icon_tmp) : "", MARKUP_NO);
      g_free(icon_tmp);
      break;
    case 'i':
      notification_replace_single_field(
    ...
    case 'p':
      ...
      notification_replace_single_field(&n->msg, &substr,
                                        n->progress != -1 ? pg : "", MARKUP_NO);
      break;
    case 'n':
      if (n->progress != -1)
...
void notification_update_text_to_render(struct notification *n) {
  ...
  if (settings.show_age_threshold >= 0 &&
    ...
    char *new_buf;
    if (hours > 0) {
      new_buf = g_strdup_printf("%s (%" G_GINT64_FORMAT "h %" G_GINT64_FORMAT
                                "m %" G_GINT64_FORMAT "s old)",
                                buf, hours, minutes, seconds);
    } else if (minutes > 0) {
      new_buf = g_strdup_printf("%s (%" G_GINT64_FORMAT "m %" G_GINT64_FORMAT
                                "s old)",
                                buf, minutes, seconds);
    } else {
      new_buf = g_strdup_printf("%s (%" G_GINT64_FORMAT "s old)", buf, seconds);
    }
